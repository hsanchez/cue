 ...
 */
public class DirectedBlockSegmentationGraph extends AbstractBlockSegmentationGraph implements
  BlockSegmentationGraph {

  private final DirectedGraph<ASTNode> implementation;

  /**
   * Creates a new BSG object.
   */
  public DirectedBlockSegmentationGraph(){
    this(new DirectedAcyclicGraph<>());
  }

  /**
   * Creates a new BSG using a DAG as the relying data structure.
   *
   * @param implementation the DAG implementation.
   */
  private DirectedBlockSegmentationGraph(DirectedGraph<ASTNode> implementation){
    super();
    this.implementation = Objects.requireNonNull(implementation);
  }

  @Override protected DirectedGraph<ASTNode> getDirectedGraph() {
    return implementation;
  }

  @Override public List<Location> irrelevantLocations(int capacity) {
    if(capacity <= 3) return ImmutableList.of();
    return generateBlackList(this, capacity);
  }

  private static List<Location> generateBlackList(DirectedBlockSegmentationGraph graph, int capacity) {
  ...
  private static boolean isPrecedenceConstraintMaintained(
    ...
    DirectedBlockSegmentationGraph graph) {
